...
///
/// This accepts only [`TdcFeature`], which prevents protein grouping and picked
/// group FDR from replacing decoy-free protein inference.
///
...
pub fn generate_protein_groups(
    ...
    features
        ...
        .for_each(|feat| {
            let pep = &db[feat.core.peptide_idx];
...
fn annotate_features(
    ...
    let peptides: FnvHashSet<PeptideIx> = features
        .par_iter()
        .filter(|f| f.core.label != -1 && f.peptide_q < threshold)
...
mod test {
    ...
    fn build_db_and_features(
        ...
        decoys: &[bool],
        q_vals: &[f32],
    ) -> (IndexedDatabase, Vec<TdcFeature>) {
    ...
    fn build_db_and_features_with_scores(
        ...
        let features: Vec<TdcFeature> = (0..proteins.len())
            .map(|ix| {
                ...
                .to_tdc();
                feature.peptide_q = q_vals[ix];
                feature.discriminant_score = scores.map(|s| s[ix]).unwrap_or(0.0);
                feature
            })
            .collect();
