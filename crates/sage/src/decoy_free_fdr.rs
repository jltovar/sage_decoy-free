...
pub fn calculate_q_values(psms: &[Feature]) -> Vec<Feature> {
    ...
    if null_scores.len() < 50 {
        ...
        new_features
            .par_iter_mut()
            .for_each(|psm| psm.spectrum_q = 1.0);
        return new_features;
    }
...
pub fn decoy_free_precursor(
    ...
    if decoy_scores.len() < 50 {
        log::error!(
            "Not enough decoy precursors ({}) to build stable LFQ null distribution.",
            decoy_scores.len()
        );
        // Set all q-values to 1.0
        peaks
            .par_iter_mut()
            .for_each(|(_, (peak, _))| peak.q_value = 1.0);
        return 0;
    }
